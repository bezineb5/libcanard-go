...
func TestReliableDeliveryEndToEnd(t *testing.T) {
	...
	// The message should have been sent with a header containing the tag
	// In a real implementation, the subscriber would receive it
	// and send an ACK back
	
	// For now, we can verify that the future was created correctly
	if future.Tag() == 0 {
		t.Log("Warning: Tag is 0 (this is OK for the first message)")
	}
...
func TestReliableDeliveryWithAssociations(t *testing.T) {
	...
	// The total count should be based on the number of associations
	...
	// The future should track acknowledgments from all associations
	// Total count should be number of associations + 1 (for multicast)
	expectedCount := pub.AssociationCount() + 1

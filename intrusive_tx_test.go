...
func makeTxTestVTable(ctx *txTestContext) Platform {
	return NewPlatform(
		func(self *Canard) int64 {
			return ctx.now
		},
		func(self *Canard, userContext any, deadline int64, ifaceIndex uint8, fd bool, extendedCANID uint32, canData []byte) bool {
...
func initTxTestCanard(ctx *txTestContext) *Canard {
	c := &Canard{}
	c.UserContext = ctx
	c.Platform = makeTxTestVTable(ctx)
